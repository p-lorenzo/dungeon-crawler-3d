...
- [ ] T016 [P] [US2] Implement weighted random selection with cooldown in `plugins/dungeon_crawler_3d/core/room_selector.gd` — function `select_weighted(pool: Array[RoomData], recent_rooms: Array[RoomData], rng: RandomNumberGenerator) -> RoomData` using cumulative distribution + recent-room penalty per research.md
- [ ] T017 [US2] Extend `DungeonBuilder` in `plugins/dungeon_crawler_3d/core/dungeon_builder.gd` — add branch generation: after main path is placed, iterate branch_count times; for each branch, pick a random main-path room as attachment point, select branch rooms from corridor/junction/dead_end pools respecting branch_depth_min/max range; integrate weighted room selector (T016); ensure branches terminate in dead-end rooms
- [ ] T018 [US2] Implement backtracking logic in `DungeonBuilder` in `plugins/dungeon_crawler_3d/core/dungeon_builder.gd` — when no valid room candidate fits a slot, pop the last placement and try the next candidate (1-step backtrack); repeat up to `max_generation_attempts` per position; signal failure if exhausted
- [ ] T019 [US2] Extend `DungeonGenerator3D` in `plugins/dungeon_crawler_3d/nodes/dungeon_generator_3d.gd` — wire topology parameters from DungeonConfig into DungeonBuilder; implement `clear()` if not already done in T014; add `generation_failed(reason: String)` signal emission on builder failure
- [ ] T020 [US2] Add branch-count vs main-path-rooms validation in `plugins/dungeon_crawler_3d/core/dungeon_builder.gd` — if `branch_count > main_path_length`, place one branch per available room and emit a partial-success note in the signal/metadata
